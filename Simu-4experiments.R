dat<-readRDS("sim1.bmi.fg.rds")#setting 1 for simulated bmi &fasting glucose GWAS
#dat<-readRDS("sim2.bmi.fg.rds") #setting 2 for simulated bmi &fasting glucose GWAS
#dat<-readRDS("sim1.ldl.cad.rds") #setting 1 for simulated LDL &CAD GWAS
#dat<-readRDS("sim2.ldl.cad.rds") #setting 2 for simulated LDL & CAD GWAS
...
S.D<-which(pi.D!=0)
S.Y<-which(pi.Y!=0)
c(sum(pi.D==0 & pi.Y==0),length(Vdy),length(Vyd))/p #propotion of null IVs, valid IVs for DtoY, valid IVs for YtoD
set.seed(123)
#data generation
